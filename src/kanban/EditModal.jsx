    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white w-full max-w-xl rounded-2xl shadow-lg p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-xl hover:bg-slate-100 inline-flex items-center justify-center" title="Zamknij"><X/></button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          {type==="priority" && (<div><label className="text-xs text-slate-500 inline-flex items-center gap-1"><Star size={14}/> Priorytet (1–5)</label>
            <input type="number" min={1} max={5} value={priority} onChange={(e)=>setPriority(e.target.value)} className="w-full border rounded-xl px-3 py-2"/></div>)}
          {type==="deadline" && (<div><label className="text-xs text-slate-500 inline-flex items-center gap-1"><CalendarDays size={14}/> Termin</label>
            <input type="datetime-local" value={due} onChange={(e)=>setDue(e.target.value)} className="w-full border rounded-xl px-3 py-2"/></div>)}
          <div className="md:col-span-2">
