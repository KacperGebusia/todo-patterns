...
export default function EditModal() {
  ...
  // Mediator: reaguj na OPEN_EDIT/CLOSE_EDIT
  useEffect(() => {
    const offOpen = uiBus.on("OPEN_EDIT", ({ task }) => {
      if (!task) return;
      setTask(task);
      setTitle(task.title);
      setType(task.type);
      ...
      const iso = task.meta?.due ? new Date(task.meta.due).toISOString() : new Date().toISOString();
      setDue(iso.slice(0, 16));
      setTagsCsv((task.meta?.tags || []).join(", "));
      setOpen(true);
    });
    const offClose = uiBus.on("CLOSE_EDIT", () => setOpen(false));
  ...
  async function onSave() {
    ...
    uiBus.emit("TOAST", { type: "success", message: "Zapisano zmiany" });
    setOpen(false);
  }
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
      <div className="bg-white w-full max-w-xl rounded-2xl shadow-lg p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => setOpen(false)} className="w-8 h-8 rounded-xl hover:bg-slate-100 inline-flex items-center justify-center" title="Zamknij">
            <X />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="md:col-span-2">
            ...
            <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full border rounded-xl px-3 py-2" placeholder="Tytuł karty" />
          </div>
          ...
          {type === "priority" && (
            <div>
              ...
              <input type="number" min={1} max={5} value={priority} onChange={(e) => setPriority(e.target.value)} className="w-full border rounded-xl px-3 py-2" />
            </div>
          )}
          ...
          {type === "deadline" && (
            <div>
              ...
              <input type="datetime-local" value={due} onChange={(e) => setDue(e.target.value)} className="w-full border rounded-xl px-3 py-2" />
            </div>
          )}

          <div className="md:col-span-2">
            <label className="text-xs text-slate-500">Tagi (CSV)</label>
            <input value={tags} onChange={(e) => setTagsCsv(e.target.value)} className="w-full border rounded-xl px-3 py-2" placeholder="tag1, tag2, tag3" />
          </div>
        </div>

        <div className="mt-4 flex justify-end gap-2">
          <button onClick={onSave} className="rounded-xl bg-slate-900 text-white px-4 py-2 inline-flex items-center gap-2">
            <Save size={16} /> Zapisz
          </button>
          <button onClick={() => setOpen(false)} className="rounded-xl bg-slate-100 px-4 py-2">Anuluj</button>
        </div>
      </div>
    </div>
  );
