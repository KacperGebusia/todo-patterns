...
export class Task {
  ...
  toJSON() { return { id: this.id, title: this.title, completed: this.completed, createdAt: this.createdAt, type: this.type, meta: this.meta, status: this.status, order: this.order }; }
}
...
export class TaskFactory { static create(kind, props = {}) { switch (kind) { case "priority": return new PriorityTask(props); case "deadline": return new DeadlineTask(props); default: return new SimpleTask(props); } }
