// [PATTERN: Singleton] • [Observer] • [Bridge] — Store główny
...
class TodoStore {
  ...
  constructor(){
    ...
    this.emitter = new Emitter(); this.errorEmitter = new Emitter(); this.backendEmitter = new Emitter();
    this.bridge = new StorageBridge(new LocalStorageBackend("factory-method-todos"));
    this.state = []; this.ready = false; this.reload();
  }
  async reload(){
    try {
      ...
      this.state = migrated; if (!ok) this.errorEmitter.emit(error); this.ready = true; this.emitter.emit(this.state);
  ...
  async update(id, patchOrWhole){
    const next = this.state.map(t => { if (t.id !== id) return t;
  ...
  async moveCard(id, toStatus, toIndex){
    const tasks = [...this.state]; const card = tasks.find(t=>t.id===id); if (!card) return;
    const fromCol = tasks.filter(t=>t.status===card.status && t.id!==id).sort((a,b)=>a.order-b.order); fromCol.forEach((t,i)=> t.order = i+1);
    const col = tasks.filter(t=>t.status===toStatus && t.id!==id).sort((a,b)=>a.order-b.order);
    col.splice(Math.min(Math.max(0,toIndex), col.length), 0, { ...card, status: toStatus }); col.forEach((t,i)=> t.order = i+1);
    ...
    this.state = next; this.emitter.emit(this.state); await this.#save();
  }
  // === Memento API ===
  getSnapshot(){ return JSON.parse(JSON.stringify(this.state)); }
  async restoreSnapshot(snapArray){ this.state = JSON.parse(JSON.stringify(snapArray || [])); await this.bridge.save(this.state); this.emitter.emit(this.state); }
  async replaceAll(next){ await this.restoreSnapshot(next); }
  // Bridge
  backendName(){ return this.bridge.name(); }
  async setBackend(kind){
    let backend; switch (kind) {
    ...
    this.bridge.setBackend(backend); this.backendEmitter.emit(this.backendName()); await this.reload();
  }
}
