...
export function addTags(task, tags){
  ...
  return { ...task, meta: { ...(task.meta||{}), tags: next, icon: task?.meta?.icon ?? "circle" } };
}
export function removeTag(task, tag){
  const next = normalizeTags(task?.meta?.tags).filter(t => t !== tag);
  return { ...task, meta: { ...(task.meta||{}), tags: next, icon: task?.meta?.icon ?? "circle" } };
}
export function setTags(task, tagsArray){
  return { ...task, meta: { ...(task.meta||{}), tags: normalizeTags(tagsArray), icon: task?.meta?.icon ?? "circle" } };
}
